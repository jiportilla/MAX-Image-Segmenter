...
DOCKER_IMAGE_BASE ?= iportilla/image.segmenter-mms
SERVICE_NAME ?=image.segmenter-mms
...
run:
	...
	docker run -d -p=$(PORT_NUM):$(PORT_NUM) --name=$(DOCKER_NAME) $(DOCKER_IMAGE_BASE)_$(ARCH):$(SERVICE_VERSION)
...
delete-mms-object:
  ...
  # register node
register-pattern:
	hzn register -p pattern-image.segmenter-mms-amd64
